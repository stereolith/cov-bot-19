...
def info_message_std(country='Germany'):
    return (
        ...
        f"    😷 {today('confirmed')} bestätigte Infizierte (+{increase_daily('confirmed')}%); "
        f"  💀 {today('deaths')} Todesfälle (+{increase_daily('confirmed')}%)\n\n"
    )

def message_country(country):
    return (
        f"  → **Heute** ({get_dates_by_type('deaths')[-1]}):\n"
        f"    😷 {today('confirmed', country)} bestätigte Infizierte (+{increase_daily('confirmed', country)}%)"
        f"  💀 {today('deaths', country)} Todesfälle (+{increase_daily('deaths', country)}%)\n"
    )
...
async def nine_nine(ctx, arg):
    if arg not in countries():
        ...
        await ctx.send(f"{arg}:\n" + message_country(country=arg))

@loop(hours=24.0)
async def info_loop():
    await covid19_channel.send(info_message_std())
